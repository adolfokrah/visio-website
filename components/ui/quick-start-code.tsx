...
export default function QuickStartCode({
  ...
  return (
    <div
      className={cn(
        "rounded-lg bg-dark-900 relative z-20 text-white flex gap-3 items-center w-max p-3",
        className,
      )}
    >
      <pre>
      ...
      {copied ? (
        ...
        <Copy size={14} className="cursor-pointer" onClick={copyCode} />
      )}
    </div>
  );
}

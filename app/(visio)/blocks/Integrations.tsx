...
const Integrations: Block<IntegrationsProps> = ({
  title,
  pageBlockId = "",
  integrations,
  footerText,
  ...
  const IntegrationsList = useMemo(() => {
    return (
      <List
        ...
        className={cn(
          "flex items-center w-max mx-auto gap-4",
          isBuilderMode && "flex-wrap",
        )}
  ...
  return (
    <div className="bg-dark-900 pb-20 ">
      <Container>
        ...
        {isBuilderMode ? (
          ...
          <Marquee className="[--duration:20s]">{IntegrationsList}</Marquee>
        )}
